...
/****************************************************************************
 * When used with open(2), O_TMPFILE allows to create an unnamed
 * temporary file.  The pathname argument specifies a directory; an
 * unnamed inode will be created in that directory's filesystem.
 * Anything written to the resulting file will be lost when the last
 ...
 * anonymous mmap).
 *
 * The prefered mechanism is memfd_create(2) (see the upper
 * description).  Then we try shm_open(2) (on Apple OS X) or open(2)
 * with O_TMPFILE (on Linux, when available). As a safe fallback, we
 * use `mkstemp/unlink`.
 *
 * TMPFILE/mkstemp is prefered over shm_open on Linux as it allows to
 ...
 * the shared heap. This is mandatory on Windows. This is optionnal on
