...
let () = assert (Sys_utils.win32 || Obj.is_int (Obj.repr Unix.stdin))
let get_handle =
  if Sys_utils.win32 then raw_get_handle else Obj.magic
let wrap_handle =
  if Sys_utils.win32 then raw_wrap_handle else Obj.magic
