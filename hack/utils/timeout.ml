...
module Alarm_timeout = struct
  ...
  let wait_for_input (ic, _) timeout =
    let fd = Unix.descr_of_in_channel ic in
    let readable, _, _ = Unix.select [fd] [] [fd] timeout in
    readable <> []
...
module Select_timeout = struct
  ...
  let in_channel_of_descr fd =
    ...
    { fd; buf; curr = 0; max = 0; on_close = None; pid = None }
  ...
  let wait_for_input { fd ; curr ; max ; _ } timeout =
    curr <> max ||
    let readable, _, _ = Unix.select [fd] [] [fd] timeout in
    readable <> []
...
module type S = sig
  ...
  val wait_for_input: in_channel -> float -> bool
end

let select = (module Select_timeout : S)
let alarm = (module Alarm_timeout : S)

include (val (if Sys_utils.win32 then select else alarm))
