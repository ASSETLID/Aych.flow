  ...
  let log_file =
    ...
    | Daemon.Log_file ->
      ...
      Hh_logger.log "About to spawn typechecker daemon. Logs will go to %s\n%!"
        (if Sys_utils.win32 then log_file else log_link);
      log_file
...
let check_and_run_loop (_: ServerArgs.options) typechecker
  ...
  try
    ...
    try
      HackEventLogger.accepted_client_fd (Handle.get_handle fd);
...
let daemon_starter options =
  ...
  Printf.eprintf "Logs will go to %s\n%!"
    (if Sys_utils.win32 then log_file_path else log_link);
  Exit_status.Ok
